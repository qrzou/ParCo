...
from dataset import dataset_VQ_uplow, dataset_TM_eval_uplow
from models import vqvae_uplow as vqvae
from models.evaluator_wrapper import EvaluatorModelWrapper

from options.get_eval_option import get_opt
import options.option_vq_uplow as option_vq
from options.option_vq_uplow import vqvae_uplow_cfg
...
args.vqvae_arch_cfg = vqvae_uplow_cfg[args.vqvae_cfg]
...
args.run_dir = os.path.join(outdir, f'{cur_run_id:05d}-{args.dataname}-{args.exp_name}-UpLow', f'VQVAE-{args.exp_name}-{desc}')
...
train_loader = dataset_VQ_uplow.DATALoader(args.dataname,
                                        args.batch_size,
                                        window_size=args.window_size,
                                        unit_length=2**args.down_t)

train_loader_iter = dataset_VQ_uplow.cycle(train_loader)

val_loader = dataset_TM_eval_uplow.DATALoader(args.dataname, False,
                                        32,
                                        w_vectorizer,
                                        unit_length=2**args.down_t)
...
Loss = losses.ReConsLossUpLow(args.recons_loss, args.nb_joints)
...
for nb_iter in range(1, args.warm_up_iter):
    ...
    pred_parts_vel = dataset_VQ_uplow.get_uplow_vel(
        pred_parts, mode=args.dataname)
    gt_parts_vel = dataset_VQ_uplow.get_uplow_vel(
