...
def evaluation_vqvae(out_dir, val_loader, net, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False) : 
    ...
    for batch in val_loader:
        ...
        for i in range(bs):
            ...
            pred_pose = val_loader.dataset.uplow2whole(pred_parts, mode=val_loader.dataset.dataset_name)
    ...
    for i in range(1):
        for batch in tqdm(val_loader):
            ...
            for k in range(bs):
                ...
                parts_index_motion = []
                for part_index, name in zip(batch_parts_index_motion, ['Upper_body', 'Lower_body']):
                ...
                #   todo: support different shared_joint_rec_mode in the parts2whole function
                pred_pose = val_loader.dataset.uplow2whole(parts_pred_pose, mode=val_loader.dataset.dataset_name)  # (B, pred_nframes, raw_motion_dim)
...
@torch.no_grad()
def evaluation_transformer_test_debug(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, best_multi, clip_model, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False) :
    """
    Comparing to evaluation_transformer_test, this _debug version
    ignores the evaluation of mmodality (which need a few days)
    """
    trans.eval()
    nb_sample = 0
