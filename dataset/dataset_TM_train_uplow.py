...
class Text2MotionDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for p_n in self.parts_name:  # ['Upper_body', 'Lower_body']
            assert len(m_token_dict[p_n]) == 1

        Upper_body        = m_token_dict['Upper_body'][0]
        Lower_body        = m_token_dict['Lower_body'][0]
        ...
        if coin:
            ...
            if coin2:
                Upper_body = Upper_body[:-1]
                Lower_body = Lower_body[:-1]
                # m_tokens = m_tokens[:-1]
            else:
                Upper_body = Upper_body[1:]
                Lower_body = Lower_body[1:]
        ...
        padded_tokens = []
        token_len = Upper_body.shape[0]  # Debug, make sure all parts motions have the same token len.
        for m_tokens in [Upper_body, Lower_body]:
        ...
        # unravel
        Upper_body, Lower_body = padded_tokens

        # caption: pure text caption
        # Upper_body, Lower_body: tokenized motion sequence
        # token_len: length, not include the End token
        return caption, Upper_body, Lower_body, token_len
